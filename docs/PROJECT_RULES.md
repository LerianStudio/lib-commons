...
│   │   ├── pacing/                 # Redis-backed outbound pacing RoundTripper (distributed, fail-closed)
...
| `net/http/idempotency` | `Store` preserves middleware-owned opaque bytes through only atomic `Acquire`, compare-safe `Complete`, and compare-safe `Release`; `NewWithStore(store, opts...)` accepts caller-provided backends and always fails closed on a missing/errored backend. Reusable adapter checks live in `idempotency/idempotencytest.Run`. |
| `net/http/idempotency` | Redis key pattern remains `<prefix><tenantID>:<idempotencyKey>`. Default prefix `"idempotency:"`, TTL 7 days, max key length 256. State, fingerprint, acquisition owner, and optional replay response are one atomic record. |
| `net/http/idempotency` | `<idempotencyKey>` comes from the `X-Idempotency` header unless `WithKeyProvider` supplies it. The provider's value drives the storage key and the fingerprint, is bounded by `WithMaxKeyLength`, takes the unkeyed branch when empty, and refuses with the pre-handler 503 on error. The middleware never writes the request header, so a handler binding the caller's raw key is unaffected by scoping. |
| `net/http/idempotency` | `<tenantID>` comes from `tmcore.GetTenantIDContext` unless `WithTenantProvider` supplies it; when set the provider is the only source and the middleware neither reads nor writes the tenant-manager context. The record is rooted at exactly the returned string (the consumer canonicalises), the provider runs once per keyed request, and an empty return or an error takes the absent-tenant branch (bypass, or the `WithRequireTenant` refusal), never a refusal of its own. |
| `net/http/idempotency` | The existing `New(conn, opts...)` Redis path remains fail-open by default. GET/OPTIONS/HEAD requests pass through. Handler error/5xx compare-safely releases only the owned acquisition. In-flight duplicate returns 409 Conflict. |
| `net/http/idempotency` | Exact replay is mandatory after a handler succeeds: marshalling, codec, completion, stale-owner, missing-response, and decode failures return 503 `IDEMPOTENCY_UNAVAILABLE`; no generic success fallback exists. A response above `WithMaxBodyCache` is not one of those faults: it is delivered to its client unchanged whatever its status. A 4xx the client-error policy releases never reaches capture and follows that policy (a resend may re-execute); an oversized response that does reach capture (raw body above `WithMaxBodyCache`, or encoded replay payload above twice that value after `WithResponseCodec`) completes its key carrying no replayable receipt, and its resend is refused 409 `IDEMPOTENCY_REPLAY_UNAVAILABLE` through `WithReplayUnavailableHandler` alone, never through the post-handler or terminal-refusal seams. A response within both limits completes with its replay receipt as usual. All four refusal codes are exported — `RefusalCodeStateUnrecognised`, `RefusalCodeRecordUnreadable` and `RefusalCodeOutcomeUnrecorded`, the trio `WithTerminalRefusalHandler` owns, plus `RefusalCodeReplayUnavailable` deliberately outside it. `WithResponseCodec` supports authenticated encryption, `WithTTLProvider` supports request-specific runtime retention, `WithFingerprintProvider` replaces the raw body as the request's identity (so a streamed or multipart route can be fingerprinted at all, at the cost of changing the digest of the same logical request when it is switched on), `WithFingerprintScopeProvider` adds a versioned, length-prefixed application scope to fingerprint comparison without changing the Redis key, `WithMaxBodyCache` bounds the cached response, and `WithClientErrorPolicy(ClientErrorPolicyRelease)` opts 4xx responses out of caching before the response is captured, and `WithClientErrorPolicyFunc` decides the same question per WRITTEN 4xx, replacing the enum entirely when set; nothing else decides whether a rejection may re-execute. `WithServerErrorPolicyFunc` decides per handler error or 5xx whether the key is released or fenced, replacing `WithServerErrorPolicy` when set, and receives the EFFECTIVE status (what the handler wrote, or the `*fiber.Error` code when it wrote nothing). `WithProcessingTTLProvider` resolves the processing lease per request, above the store deadline, applied at the next acquisition and never to a lease already held, falling back to `WithProcessingTTL` and then to the retention. Omitting the scope provider preserves legacy fingerprint vectors byte-for-byte; a configured provider remains in the scoped domain even when it returns an empty string. In-flight 409 is the only response that sets `Retry-After: 1`. |
| `net/http/pacing` | `NewPacer(conn, prefix, opts...) (*Pacer, error)` and `NewRoundTripper(next, pacer, BucketsFunc) (*RoundTripper, error)` return errors rather than nil primitives; a nil `next` falls back to `http.DefaultTransport`. Buckets come only from `TenantBucket` (canonicalized via `tenant-manager/core.CanonicalTenantID`) or `InstitutionBucket` (identifier grammar only, separate namespace); the zero `Bucket` is rejected. |
| `net/http/pacing` | `Acquire(ctx, buckets...)` charges every supplied bucket in one Lua evaluation or charges none, and a refusal writes no bucket. State is the LAST GRANT timestamp with a flat one-day key lifetime, so a rate raised at runtime shortens an in-flight wait and a rate lowered at runtime cannot be expired. Time is the Redis `TIME` command plus a per-prefix high-water mark; a backwards clock refuses. Key pattern is `pacing:{<prefix>}:{tenant\|inst}:<sha256-8-hex>` and identities are never stored, logged, or returned in clear. Every failure mode fails closed — there is no fail-open option and no burst option. |
...
| `outbox` | `OutboxRepository` contract: `Create`, `CreateWithTx`, `ListPending`, `ListPendingByType`, `ListTenants`, `GetByID`, `MarkPublished`, `MarkFailed`, `ListFailedForRetry`, `ResetForRetry`, `ResetStuckProcessing`, `MarkInvalid`. Optional capabilities: `IdempotentWriter`, `MultiTypePendingRepository`, `TransactionalBatchWriter`, `PublishedPurger` (`DeletePublishedBefore`, required by dispatcher retention), `PublishedTenantLister` (`ListTenantsWithPublishedBefore`), `InvalidPurger` (`DeleteInvalidBefore`, `ListTenantsWithInvalidBefore`, required by `WithRetentionInvalid`; postgres only). Pool-per-tenant, schema-per-tenant and mongo with a tenant database resolver sweep each known tenant as it is dispatched; column-per-tenant and row-scoped mongo (tenant field, no database resolver) implement `PublishedTenantLister`, so once per sweep interval the dispatcher also sweeps every tenant holding aged PUBLISHED events, idle tenants included. The sweep interval is per dispatcher instance (N replicas, up to N batches per scope per interval; deletes are idempotent). |
| `outbox` | `MultiTypePendingRepository` is an additive atomic-claim capability. When priority event types are configured, supporting repositories treat them as the exclusive dispatcher claim scope and never fall back to unscoped stuck, failed, or pending claims; older adapters preserve the existing per-type priority behavior. Caller event-type order is priority; FIFO is `created_at ASC, id ASC` within each type. |
...
| `outbox/postgres` | `ListPendingByTypes` claims all selected rows and marks them `PROCESSING` in one tenant-scoped transaction using `FOR UPDATE SKIP LOCKED`; concurrent claimers cannot receive the same event. |
