...
| `outbox` | `OutboxRepository` contract: `Create`, `CreateWithTx`, `ListPending`, `ListPendingByType`, `ListTenants`, `GetByID`, `MarkPublished`, `MarkFailed`, `ListFailedForRetry`, `ResetForRetry`, `ResetStuckProcessing`, `MarkInvalid`. Optional capabilities: `IdempotentWriter`, `MultiTypePendingRepository`, `TransactionalBatchWriter`, `PublishedPurger` (`DeletePublishedBefore`, required by dispatcher retention). Retention sweeps a scope only in a pass where it is dispatched: pool-per-tenant and schema-per-tenant sweep each known tenant; column-per-tenant and mongo tenant-field modes sweep tenants with outstanding work at discovery time, so idle tenants wait until they have work again. The sweep interval is per dispatcher instance (N replicas, up to N batches per scope per interval; deletes are idempotent). |
