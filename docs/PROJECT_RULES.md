...
| `outbox` | `MultiTypePendingRepository` is an additive atomic-claim capability. Dispatcher priority selection uses it when available and preserves the existing per-type fallback for older adapters. Caller event-type order is priority; FIFO is `created_at ASC, id ASC` within each type. |
