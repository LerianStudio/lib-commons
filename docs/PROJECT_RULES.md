...
| `outbox` | `OutboxRepository` contract: `Create`, `CreateWithTx`, `ListPending`, `ListPendingByType`, `ListTenants`, `GetByID`, `MarkPublished`, `MarkFailed`, `ListFailedForRetry`, `ResetForRetry`, `ResetStuckProcessing`, `MarkInvalid`. Optional capabilities: `IdempotentWriter`, `MultiTypePendingRepository`, `TransactionalBatchWriter`, `PublishedPurger` (`DeletePublishedBefore`, required by dispatcher retention). |
