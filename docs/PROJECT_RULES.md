...
| `certificate` | Private key parsing order: PKCS#8 → PKCS#1 → EC. Key file mode must be 0640 or stricter — group-read is allowed (K8s Secret + `fsGroup` for non-root pods), group-write and any `other` bit are rejected (mask `0o027`, enforced at load time). Public key match validated against cert at load and rotate. |
