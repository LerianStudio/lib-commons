...
func TestServerManager_TypedNilLoggerSafe(t *testing.T) {
	...
	select {
	case err := <-done:
		assert.NoError(t, err, "typed-nil logger should be normalized to nop logger")
	case <-time.After(5 * time.Second):
		t.Fatal("Test timed out")
	}
}

func TestShutdownHook_RunsAfterGRPCGracefulStop(t *testing.T) {
	logger := &recordingLogger{}
	grpcServer := grpc.NewServer()
	shutdownChan := make(chan struct{})
	hookObservedGRPCStopped := make(chan bool, 1)

	sm := server.NewServerManager(nil, nil, logger).
		WithGRPCServer(grpcServer, "127.0.0.1:0").
		WithShutdownChannel(shutdownChan).
		WithShutdownHook(func(context.Context) error {
			msgs := logger.getMessages()
			for _, msg := range msgs {
				if msg == "gRPC server stopped gracefully" {
					hookObservedGRPCStopped <- true
					return nil
				}
			}

			hookObservedGRPCStopped <- false
			return nil
		})
	...
	select {
	case observed := <-hookObservedGRPCStopped:
		assert.True(t, observed, "shutdown hook must run after gRPC graceful stop completes")
	case <-time.After(5 * time.Second):
		t.Fatal("Test timed out waiting for shutdown hook")
	}
...
func TestShutdownHook_RunsAfterHTTPShutdown(t *testing.T) {
	...
	sm := server.NewServerManager(nil, nil, logger).
		...
		WithShutdownHook(func(context.Context) error {
			msgs := logger.getMessages()
			for _, msg := range msgs {
				if msg == "Shutting down HTTP server..." {
					hookObservedHTTPShutdown <- true
					return nil
				}
			}

			hookObservedHTTPShutdown <- false
			return nil
		})
	...
	select {
	case observed := <-hookObservedHTTPShutdown:
		assert.True(t, observed, "shutdown hook must run after HTTP shutdown begins")
	case <-time.After(5 * time.Second):
		t.Fatal("Test timed out waiting for shutdown hook")
	}
