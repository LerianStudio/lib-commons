...
// -------------------------------------------------------------------
// GetTenantValue — validation guards (before DB I/O)
// -------------------------------------------------------------------

func TestStore_GetTenantValue_EmptyTenantID(t *testing.T) {
	t.Parallel()

	s := &Store{} // not closed, not nil
	_, _, err := s.GetTenantValue(context.Background(), "", "ns", "key")
	require.Error(t, err)
	assert.Contains(t, err.Error(), "tenantID must not be empty")
}

func TestStore_GetTenantValue_SentinelGlobal(t *testing.T) {
	t.Parallel()

	s := &Store{}
	_, _, err := s.GetTenantValue(context.Background(), "_global", "ns", "key")
	require.Error(t, err)
	assert.Contains(t, err.Error(), "_global")
}

// -------------------------------------------------------------------
// SetTenantValue — validation guards
// -------------------------------------------------------------------
...
// -------------------------------------------------------------------
// DeleteTenantValue — validation guards
// -------------------------------------------------------------------
