...
func testDeleteTenantValueReturnsMissing(t *testing.T, factory Factory) {
	...
	s := newStore(t, factory)
	ctx := context.Background()
	entry := store.Entry{Namespace: fixtureNamespace, Key: fixtureKey, Value: []byte(`0.05`), UpdatedBy: fixtureActorAdmin}

	if err := s.SetTenantValue(ctx, fixtureTenantA, entry); err != nil {
		t.Fatalf("SetTenantValue: %v", err)
	}

	if err := s.DeleteTenantValue(ctx, fixtureTenantA, fixtureNamespace, fixtureKey, fixtureActorAdmin); err != nil {
...
func testDeleteTenantValueIsIdempotent(t *testing.T, factory Factory) {
	...
	if err := s.DeleteTenantValue(ctx, "tenant-missing", fixtureNamespace, fixtureKey, fixtureActorAdmin); err != nil {
...
func testListTenantsForKeySorted(t *testing.T, factory Factory) {
	...
	if err := s.Set(ctx, store.Entry{Namespace: fixtureNamespace, Key: fixtureKey, Value: []byte(`0.01`), UpdatedBy: fixtureActorAdmin}); err != nil {
...
func testListTenantOverridesFiltersGlobals(t *testing.T, factory Factory) {
	...
	for _, e := range []store.Entry{
		{Namespace: fixtureNamespace, Key: "rate_limit", Value: []byte(`100`), UpdatedBy: fixtureActorAdmin},
		{Namespace: fixtureNamespace, Key: "log.level", Value: []byte(`"info"`), UpdatedBy: fixtureActorAdmin},
	} {
...
func testTenantSubscribeReceivesDeleteEvent(t *testing.T, factory Factory, settle time.Duration) {
	...
	seed := store.Entry{Namespace: fixtureNamespace, Key: fixtureKey, Value: []byte(`0.05`), UpdatedBy: fixtureActorAdmin}
...
func testDeleteTenantValueNoOpEmitsNoEvent(t *testing.T, factory Factory, settle time.Duration) {
	...
	if err := s.DeleteTenantValue(ctx, "tenant-never", fixtureNamespace, fixtureKey, fixtureActorAdmin); err != nil {
