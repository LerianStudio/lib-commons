...
func backfillTenantID(ctx context.Context, coll *mongo.Collection) error {
	filter := bson.D{{Key: mongoFieldTenantID, Value: bson.D{{Key: mongoOperatorExists, Value: false}}}}
	update := bson.D{{Key: mongoOperatorSet, Value: bson.D{{Key: mongoFieldTenantID, Value: store.SentinelGlobal}}}}
