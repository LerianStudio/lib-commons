...
// It fetches credentials from Pool Manager and caches connections.
type Pool struct {
	client  *Client
	service string
	module  string
...
// createConnection fetches config from Pool Manager and creates a connection.
...
// IsMultiTenant returns true if the pool is configured with a Pool Manager client.
