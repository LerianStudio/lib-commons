...
- `commons/net/http/ratelimit`: Redis-backed distributed rate limiting middleware for Fiber — `New(conn, opts...)` returns a `*RateLimiter` (nil when disabled, nil-safe for pass-through), fixed-window counter via atomic Lua script (INCR + PEXPIRE); `ForTier(tierName)` applies named-tier rate limiting per route or group with automatic single-tenant/multi-tenant resolution; `WithTenantResolver(resolver, extractTenantID)` enables multi-tenant mode where tenant-specific limits are resolved from cache with local fallbacks; `WithTierFallback(tier)` registers custom tiers by name; preset tiers (`DefaultTier` / `AggressiveTier` / `RelaxedTier`) configurable via env vars; `TierFromMethod` maps HTTP methods to canonical tier names (`"aggressive"` for writes, `"default"` for reads); legacy API (`WithRateLimit(tier)` for static tiers, `WithDynamicRateLimit(TierFunc)` for per-request selection, `MethodTierSelector` / `TenantMethodTierSelector`, `WithDefaultRateLimit`) remains fully supported; identity extractors (`IdentityFromIP` / `IdentityFromHeader` / `IdentityFromIPAndHeader` — uses `#` separator to avoid conflict with IPv6 colons), fail-open/fail-closed policy, `WithOnLimited` callback, and standard `X-RateLimit-*` / `Retry-After` headers; also exports `RedisStorage` (`NewRedisStorage`) for use with third-party Fiber middleware
