...
#### Idempotency v6.2 rolling upgrade

Use `WithRedisLegacyBridge()` only with the built-in Redis constructor while v6.2 and current pods share a deployment. When the validated original tenant spelling differs from its trusted canonical spelling, one atomic Lua transition observes or creates both namespaces: the original namespace receives the exact v6.2 `processing:<fingerprint>` / `complete:<fingerprint>` marker plus companion `:response`, and the canonical namespace receives the current owner-fenced JSON record with the same replay response. This blocks v6.2, bridge, and bridge-disabled current readers from acquiring the same logical operation. Invalid or mismatched original identities fail with 503 before Redis.

The bridge supports standalone and sentinel Redis with the default plaintext response codec. `NewWithStore`, Redis Cluster, and any custom `ResponseCodec` fail closed before Redis or the protected handler. Cluster cannot atomically mutate the legacy keys across hash slots, and v6.2 cannot decode transformed or encrypted response envelopes.

Deploy in two steps: first enable the bridge on every new pod, drain every v6.2 pod, and wait the maximum configured idempotency TTL. Every dual-written operation already has canonical JSON, so the second deployment can disable the bridge without opening a fresh acquisition; writes then return to the current single-record format and existing requests replay from canonical state. Keep the legacy reader for one further maximum TTL after the last bridge writer stops. Owner fencing protects completion and release attempts from stale new pods, but cannot fence stale v6.2 pods because v6.2 never wrote owner tokens; keep the minimum TTL longer than the protected operation and retry window during the mixed-version deployment.
